...
L'objectif des étapes **Etant donné que** est de **mettre le système dans un état
...
des interactions de l'utilisateur dans les étapes "Etant donné".
...
.. note::
    ...
    Voici deux bons exemples d'utilisation de **Etant donné** :

    * Pour créer des enregistrements ou pour initialiser la base de données :

        .. code-block:: gherkin

            Etant donné qu'il n'y à aucun utilisateur sur le site
            Etant donné que la base de données est vierge
...
Et nous recommandons aux utilisateurs de Symfony d'utiliser les étapes "Etant
...
.. code-block:: gherkin
    ...
    Etant donné qu'il y a les utilisateurs:
...
* Vérifier que quelque chose, relatif au "Etant donné" + "Quand", est (ou n'est pas)
